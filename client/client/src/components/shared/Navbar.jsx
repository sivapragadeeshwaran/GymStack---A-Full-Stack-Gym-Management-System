...
export default function Navbar({ onNavigate, activeSection }) {
  ...
  const { authUser: contextAuthUser, logout: contextLogout } = useAuth();

  // Initialize auth state from context and verify token on component mount
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        // If context already has authUser, use it
        if (contextAuthUser) {
          setAuthUser(contextAuthUser);
  ...
  useEffect(() => {
    if (isInitialized) {
      if (contextAuthUser) {
        ...
        // If context auth user is null, clear local state
        setAuthUser(null);
      }
    }
  }, [contextAuthUser, isInitialized]);
  ...
  return (
    <div
      className="relative flex size-full flex-col overflow-x-hidden bg-[#1a1a1a] pb-24"
        ...
        <header className="flex items-center justify-between border-b border-[#363636] px-4 sm:px-10 py-3 fixed top-0 left-0 right-0 z-30 bg-[#1a1a1a]">
          ...
          <div className="hidden md:flex flex-1 justify-end gap-8 items-center">
            <div className="flex items-center gap-9">
              {navItems.map(({ label, key }) => (
                <button
                  ...
                  className={`text-sm font-medium text-white relative pb-1 ${
                    activeSection === key
            ...
            {!authUser ? (
              <button
                ...
                className="flex h-10 min-w-[84px] transition active:scale-95 border-2 border-[#474747] items-center justify-center rounded-full bg-black px-4 text-sm font-bold text-white"
              >
                Login
              </button>
            ) : (
              ...
              >
                <div
                  className="flex items-center gap-2 cursor-pointer rounded-full border border-transparent bg-[#222] px-3 py-2"
                ...
                {showLogout && (
                  <div
                    className="transition-all duration-200 absolute right-0 mt-2 w-[120px] bg-[#1f1f1f] border border-[#444] rounded-md shadow-lg z-50"
                    onMouseEnter={handleDropdownMouseEnter}
                    onMouseLeave={handleDropdownMouseLeave}
                  >
                    <button
                      onClick={handleLogout}
                      className="w-full text-left px-4 py-2 text-sm text-[#f1f1f1] hover:bg-[#2d2d2d] rounded-b-md"
                    >
                      Logout
                    </button>
                  </div>
          ...
          <div className="md:hidden flex items-center gap-3">
            {!authUser ? (
              <button
                onClick={() => navigate("/login")}
                className="flex h-9 px-3 py-2 rounded-full bg-black text-sm font-bold text-white border border-[#474747]"
              >
                Login
              </button>
            ) : (
              <div className="relative group">
                <div
                  className="w-8 h-8 flex items-center justify-center rounded-full bg-white text-black font-bold cursor-pointer"
                  onClick={handleAvatarClick}
                >
                  {authUser.username?.[0]?.toUpperCase() || "U"}
                </div>
          ...
          <div className="md:hidden flex flex-col gap-4 px-6 pb-4 bg-[#1a1a1a] pt-[80px]">
            ...
            {!authUser ? (
              <button
                ...
                className="w-full h-10 rounded-full bg-black px-4 text-sm font-bold text-white"
              >
                Login
              </button>
            ) : (
